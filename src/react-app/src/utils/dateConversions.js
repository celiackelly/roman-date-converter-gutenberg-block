...
function formatYear(year, era, yearDisplayOption) {
	...
	if (yearDisplayOption === "secularNotation") {
		notation =
			era === "B.C. / B.C.E." ? "ante aeram vulgarem" : "aerae vulgaris";
	}

	if (yearDisplayOption === "christianNotation") {
		notation =
			era === "B.C. / B.C.E." ? "ante Christum natum" : "post Christum natum";
	}
...
export function normalizeFormData(formData) {
	...
	normalizedData.displayYear
		? (normalizedData.displayYear = true)
		: (normalizedData.displayYear = false);
...
export function abbreviateDate(dateString) {
	...
	abbreviatedDate = abbreviatedDate.replace(
		/(prid|Kal|Id|Non|Ian|Feb|Mart|Apr|Aug|Sept|Oct|Nov|Dec)\w+/g,
		"$1."
	);
...
export function abbreviateYear(yearString) {
	...
	abbreviatedYear = abbreviatedYear.replace(
		"ante aeram vulgarem",
		"ante aer. vulg."
	);
...
export function outputFormattedRomanDate(formData) {
	const { day, displayYear, era, month, year, yearDisplayOption } = formData;

	const romanDate = convertToRomanDate(day, month, year);

	const formattedYear = displayYear
		? formatYear(year, era, yearDisplayOption)
		: "";
